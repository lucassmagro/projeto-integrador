...
const NAV = [
    ...
    { to: "/busca", label: "Buscar Paciente", icon: "bi-search", match: (p) => p === "/busca" },
    { to: "/registro", label: "Novo Registro", icon: "bi-file-earmark-plus", match: (p) => p.startsWith("/registro") },
    { to: "/retificacao", label: "Retificação", icon: "bi-pencil", match: (p) => p.startsWith("/retificacao"), hideTab: true },
];

function crumbsFor(pathname) {
    if (pathname === "/") return [{ label: "Início" }];
    if (pathname === "/busca") return [{ to: "/", label: "Início" }, { label: "Consulta de Histórico Clínico" }];
    if (pathname.startsWith("/registro")) return [{ to: "/", label: "Início" }, { to: "/busca", label: "Histórico Clínico" }, { label: "Novo Registro Clínico" }];
    if (pathname.startsWith("/retificacao")) return [{ to: "/", label: "Início" }, { to: "/busca", label: "Histórico Clínico" }, { label: "Retificação de Registro" }];
            ...
            <nav className="appbar">
                <div className="shell-width appbar-inner">
                    <Link to="/" className="brand" onClick={() => setOpen(false)}>
                        <span className="brand-mark"><i className="bi bi-clipboard2-pulse"></i></span>
